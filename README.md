...
npm install react-native-inappbrowser-rebsorn
...
yarn add react-native-inappbrowser-rebsorn
...
### getStatus (Support from version 0.0.2-alpha)

```javascript
await wepin.getStatus()
```

The `getStatus()` method returns user status.
...
- `Promise` \<IStatus>
  - This method returns a `Promise` object resolved with object of `status`.
    - Type of `status` is assigned at [`@wepin/types`](https://github.com/WepinWallet/wepin-js-sdk-types) as `IStatus`(Support from version X.0)
    - `status` \<IStatus>
      - `login` \<boolean>
      - `loginStatus` \<"completed"|"register_required">
  - - If user is logged in, `login` is set to true, and `loginStatus` is set to `"complete"` or `"register_required"`.
      - The `loginStatus` value is configured as follows.
        - `"completed"`: Login to wepin, join to app.
        - `"register_required"` : Login to wepin, but do not join the app
    - If user is not logged in, `login` is set to false, and `loginStatus` does not exist.
  - Example
    ```javascript
    {
      login: true,
